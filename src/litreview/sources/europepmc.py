...
class EuropePmcAdapter(SourceAdapter):
    ...
    def search(self, query: str, window: DateWindow, max_results: int = 10) -> list[PaperRecord]:
        ...
        response = self.client.get(
            self.api_url,
            params={"query": f"({query}) AND {date_clause}", "format": "json", "pageSize": max_results},
        )
    ...
    def search_author(
        ...
        response = self.client.get(
            self.api_url,
            params={"query": f'AUTH:"{author}" AND {date_clause}', "format": "json", "pageSize": max_results},
        )
        response.raise_for_status()
        results = response.json().get("resultList", {}).get("result", [])
        return [record for item in results if (record := self._normalize(item))]
    ...
    def _normalize(self, item: dict[str, Any]) -> PaperRecord | None:
        publication_date = parse_date(item.get("firstPublicationDate") or item.get("firstIndexDate"))
        ...
        return PaperRecord(
            ...
            year=int(item["pubYear"]) if str(item.get("pubYear", "")).isdigit() else publication_date.year,
