...
def test_registry_loads() -> None:
    ...
    assert any(item.name == "Example Author" for item in registry.watchlist)
    assert any(item.type == "lab" and len(item.related_topics) == 2 for item in registry.watchlist)


def test_duplicate_topic_ids_fail(tmp_path: Path) -> None:
    data = yaml.safe_load(Path("registry.example.yaml").read_text())
